...
export default class Paginator extends PureComponent {
  ...
  nextClicked = () => {
    ...
    if (cached && cachedPages[currentPage]) {
      ...
      timeSlotsCollection.nextPage().then(nextTimeSlotsCollection => {
  ...
  prevClicked = () => {
    ...
    if (cached) {
      ...
      timeSlotsCollection.prevPage().then(prevTimeSlotsCollection => {
  ...
  loadNextTimeSlotPages = (numberOfPages, collection) => {
    ...
    collection.nextPage().then(nextPage => {
  ...
  render() {
    ...
    return (
      <section className="time-slots-paginator">
        <Pagination className={className}>
          ...
          <PaginationItem
            disabled={
              !timeSlotsCollection.hasNextPage() ||
              (loading && currentPage >= cachedPages.length - 1)
            }
          >
